...
const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section id="testimonials" className="py-16 md:py-24 bg-[#F5EFE8]">
      <div className="max-w-container mx-auto px-4 md:px-8">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-3xl p-8 md:p-12 shadow-md border border-[#D6C8AF]/30 relative">
            <span className="font-display text-6xl md:text-7xl text-secondary/40 leading-none absolute top-6 left-8 md:left-10">
              &ldquo;
            </span>

            <blockquote className="font-display italic text-lg md:text-xl lg:text-2xl text-[#5D2413] leading-relaxed pt-8 mb-8">
              &ldquo;{active.quote}&rdquo;
            </blockquote>

            <div className="border-t border-[#D6C8AF]/40 pt-6">
              <p className="font-bold text-[#5D2413] text-base">{active.name}</p>
            ...
            {items.map((item, index) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all duration-300 ${
                  activeIndex === index
                    ? 'w-8 bg-primary'
                    : 'w-2.5 bg-[#D6C8AF] hover:bg-[#D6C8AF]/80'
                }`}
              />
            ))}
          </div>
